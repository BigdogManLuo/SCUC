...
def calculateCosts(P_ES_ch,P_ES_dch,U_ch,U_dch,P_unit,U_unit,):
    ...
    for t in range(24):
        for i in range(P_unit.shape[0]):
            ...
            #计算成本区间上下限
            P_unit_mins,P_unit_maxs=getSegmentedPoints(num_seg=N_unit_segs,deltaP=bid_capacity.iloc[i,1],P_min=unitdata['最小出力(MW)'][i])
...
#---------------------------Parameters-----------------------------------------#
instance_num=60
bid_capacity = txt_to_dataframe(read_txt(f'data/instances/{instance_num}/bidcapacity.txt'))
bid_price= txt_to_dataframe(read_txt(f'data/instances/{instance_num}/bidprice.txt'))
section= txt_to_dataframe(read_txt(f'data/instances/{instance_num}/section.txt'))
load= txt_to_dataframe(read_txt(f'data/instances/{instance_num}/slf.txt'))
stbidcapactiy= readStorageBidInfo(f'data/instances/{instance_num}/stbidcapacity.txt')
stbidprice= readStorageBidInfo(f'data/instances/{instance_num}/stbidprice.txt')
storagebasic= txt_to_dataframe(read_txt(f'data/instances/{instance_num}/storagebasic.txt'))
unitdata= txt_to_dataframe(read_txt(f'data/instances/{instance_num}/unitdata.txt'))
gen_senses,load_sense,branch=parse_log_file(f'data/instances/{instance_num}/branch_1.log')
...
for i in range(N_units):
    a_unit[i],b_unit[i]=getSegmentedCostInfo(prices=bid_price.iloc[i,2:],deltaP=bid_capacity.iloc[i,1],P_min=unitdata['最小出力(MW)'][i])
...
a_ES,b_ES=getSegmentedCostInfo(stbidprice[2:],deltaP=deltaP_ES,P_min=storagebasic['最小发电功率（MW）'][0]) #储能成本分段线性化
P_ES_mins,P_ES_maxs=getSegmentedPoints(num_seg=N_ES_segs,deltaP=deltaP_ES,P_min=storagebasic['最小发电功率（MW）'][0]) #储能功率分段的上下限
C0_ES=stbidprice[2]
...
#---------------------------Variables-----------------------------#
#Unit Data
P_unit=cp.Variable((N_units,T))
U_unit=cp.Variable((N_units,T),boolean=True) #开停机state
V_unit=cp.Variable((N_units,T),boolean=True) #启动indicator
W_unit=cp.Variable((N_units,T),boolean=True) #停机indicator
P_unit_segs=[cp.Variable((N_units,T)) for j in range(N_unit_segs)] #火电机组分段功率
PF_unit=cp.Variable((N_restricted_branches,T)) #火电机组对断面的潮流
Cost_unit_start=cp.Variable((N_units,T))
Cost_unit_opr=cp.Variable((N_units,T))
Cost_unit_opr_per_seg=[cp.Variable((N_units,T)) for j in range(N_unit_segs)] #火电机组成本分段线性化变量

#Storage Data
U_ch = cp.Variable((N_ESs, T), boolean=True) #充电state
U_dch = cp.Variable((N_ESs, T), boolean=True) #放电state
U_ES=cp.Variable((N_ESs,T),boolean=True) #停机state
Z_ch=cp.Variable((N_ESs,T),boolean=True) #充电indicator
Z_dch=cp.Variable((N_ESs,T),boolean=True) #放电indicator
Z_ES=cp.Variable((N_ESs,T),boolean=True) #停机indicator
P_ES_ch=cp.Variable((N_ESs,T)) #充电功率
P_ES_dch=cp.Variable((N_ESs,T)) #放电功率
P_ES_dch_segs=[cp.Variable((N_ESs,T)) for j in range(N_ES_segs)] #储能机组分段功率
PF_ES=cp.Variable((N_restricted_branches,T)) #储能机组对断面的潮流
ES=cp.Variable((N_ESs,T+1)) #储能容量剩余
Cost_ES_ch=cp.Variable((N_ESs,T)) #储能充电成本
Cost_ES_dch=cp.Variable((N_ESs,T)) #储能放电成本
Cost_ES_dch_per_seg=[cp.Variable((N_ESs,T)) for j in range(N_ES_segs)] #储能成本分段线性化变量
...
for i in range(N_units):
    ...
    for t in range(T):
        ...
        if t==0: #初始时刻的启停机状态
            ...
            constraints+=[V_unit[i,t]<=1-unitdata['初始状态(1开机,0停机)'][i]]
            constraints+=[V_unit[i,t]<=U_unit[i,t]]
            
            constraints+=[W_unit[i,t]>=unitdata['初始状态(1开机,0停机)'][i]-U_unit[i,t]]
            constraints+=[W_unit[i,t]<=1-U_unit[i,t]]
            constraints+=[W_unit[i,t]<=unitdata['初始状态(1开机,0停机)'][i]]
        
        if t>0:
            constraints+=[V_unit[i,t]>=U_unit[i,t]-U_unit[i,t-1]]
            constraints+=[V_unit[i,t]<=1-U_unit[i,t-1]]
            constraints+=[V_unit[i,t]<=U_unit[i,t]]
            
            constraints+=[W_unit[i,t]>=U_unit[i,t-1]-U_unit[i,t]]
            constraints+=[W_unit[i,t]<=1-U_unit[i,t]]
            constraints+=[W_unit[i,t]<=U_unit[i,t-1]]
        ...
        #仅调试
        #constraints+=[Cost_unit_opr[i,t]==np.mean(a_unit[i])*P_unit[i,t]+np.mean(b_unit[i])*U_unit[i,t]]
        
        
        #运行成本分段线性化
        for j in range(N_unit_segs):
            
            constraints+=[P_unit_segs[j][i,t]>=0]
            constraints+=[P_unit_segs[j][i,t]<=deltaP_unit[i]]

        #每段功率之和等于总功率
        constraints+= [P_unit[i,t]==sum([P_unit_segs[j][i,t] for j in range(N_unit_segs)])+unitdata['最小出力(MW)'][i]*U_unit[i,t]]
            
        #总成本
        constraints+=[Cost_unit_opr[i,t]==sum(a_unit[i][j]*P_unit_segs[j][i,t] for j in range(N_unit_segs))+U_unit[i,t]*C0_unit[i]]
...
for i in range(N_ESs):
    ...
    for t in range(T):
        ...
        constraints += [ES[i, t] <= storagebasic['最大容量（MWh）'][i]]
        constraints += [ES[i, t] >= 0]
        ...
        if t==0:
            ...
            constraints += [Z_ES[i, t] == 0] #第一时刻停机也不会激活停机indicator  
            
        if t > 0:
            constraints += [Z_ch[i, t] >= U_ch[i, t] - U_ch[i, t - 1]]
            constraints += [Z_ch[i, t] <= 1 - U_ch[i, t - 1]]
            constraints += [Z_ch[i, t] <= U_ch[i, t]]
            
            constraints += [Z_dch[i, t] >= U_dch[i, t] - U_dch[i, t - 1]]
            constraints += [Z_dch[i, t] <= 1-U_dch[i, t - 1]]     
            constraints += [Z_dch[i, t] <= U_dch[i, t]]     
            
            constraints += [Z_ES[i, t] >= U_ES[i, t] - U_ES[i, t - 1]]
            constraints += [Z_ES[i, t] <= 1 - U_ES[i, t - 1]]
            constraints += [Z_ES[i, t] <= U_ES[i, t]]
        ...
        #充电成本
        constraints+= [Cost_ES_ch[i,t]==-stbidprice[0]*U_ch[i,t]]
        
        
        #放电成本分段线性化
        for j in range(N_ES_segs):
            constraints+=[P_ES_dch_segs[j][i,t]>=0]
            constraints+=[P_ES_dch_segs[j][i,t]<=deltaP_ES]

        #每段功率之和等于总功率
        constraints+= [P_ES_dch[i,t]==sum([P_ES_dch_segs[j][i,t] for j in range(N_ES_segs)])+storagebasic['最小发电功率（MW）'][i]*U_dch[i,t]]

        #总成本
        constraints+=[Cost_ES_dch[i,t]==sum(a_ES[j]*P_ES_dch_segs[j][i,t] for j in range(N_ES_segs))+U_dch[i,t]*C0_ES]
...
for t in range(T):
    ...
    #系统备用约束
    constraints += [sum(U_unit[i,t]*unitdata['最大出力(MW)'][i]-P_unit[i, t] for i in range(N_units)) >= 0.1*load['系统负荷大小（MW）'][t]]
    ...
    for j in range(N_restricted_branches):
        ...
        #火电机组对断面的潮流
        constraints+=[
            PF_unit[j,t]==sum(P_unit[i,t]*gen_senses[f'{i+1}对所列支路潮流的灵敏度值'][branch_idx] for i in range(N_units))
        ]

        #储能机组对断面的潮流
        constraints+=[
            PF_ES[j,t]==sum((P_ES_dch[i,t]-P_ES_ch[i,t])*gen_senses[f'{len(unitdata)}对所列支路潮流的灵敏度值'][branch_idx] for i in range(N_ESs))
        ]
...
obj=cp.Minimize(cp.sum(Cost_unit_start)+cp.sum(Cost_unit_opr)+
...
if prob.status=='optimal':
    ...
    #print 成本
    print('火电机组启动成本:',np.sum(Cost_unit_start.value))
    print('火电机组运行成本:',np.sum(Cost_unit_opr.value))
    print('储能机组充电成本:',np.sum(Cost_ES_ch.value))
    print('储能机组放电成本:',np.sum(Cost_ES_dch.value))
    print('总成本:',prob.value)
    print('---------------------------------')
    
    #通过成本函数计算成本
    Cost_ES_ch,Cost_ES_dch,Cost_unit_start,Cost_unit_opr=calculateCosts(P_ES_ch.value[0],P_ES_dch.value[0],U_ch.value[0],U_dch.value[0],P_unit.value,U_unit.value)
    print('校验火电机组启动成本:',np.sum(Cost_unit_start))
    print('校验火电机组运行成本:',np.sum(Cost_unit_opr))
    print('校验储能机组充电成本:',np.sum(Cost_ES_ch))
    print('校验储能机组放电成本:',np.sum(Cost_ES_dch))
    print('校验总成本:',np.sum(Cost_unit_start)+np.sum(Cost_unit_opr)+np.sum(Cost_ES_ch)+np.sum(Cost_ES_dch))
    print('---------------------------------')


    #校验外部最优解对应的成本
    Vars=getSols(f'data/instances/{instance_num}/solution.sol')
    Vars.pop('U_ES')
    Cost_ES_ch,Cost_ES_dch,Cost_unit_start,Cost_unit_opr=calculateCosts(**Vars)
    print('校验火电机组启动成本:',np.sum(Cost_unit_start))
    print('校验火电机组运行成本:',np.sum(Cost_unit_opr))
    ...
    Unit_status['启停成本']=unitdata['启动成本（元）']
