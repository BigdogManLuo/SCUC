...
P_ES_mins,P_ES_maxs=getSegmentedPoints(num_seg=N_ES_segs,deltaP=stbidcapactiy[1],P_min=storagebasic['最小发电功率（MW）'][0]) #储能分段功率的上下限
...
L_unit=[cp.Variable((N_units,T),boolean=True) for j in range(N_unit_segs)] #火电机组属于哪个分段indicator
...
L_ES=[cp.Variable((N_ESs,T),boolean=True) for j in range(N_ES_segs)] #储能属于哪个分段indicator
...
for i in range(N_units):
    ...
    for t in range(T):
        ...
        #仅调试：等效线性成本
        constraints+=[Cost_unit_opr[i,t]==P_unit[i,t]*np.mean(a_unit)+U_unit[i,t]*np.mean(b_unit)]
        
        '''
        #分段功率上下限约束
        constraints+=[P_unit[i,t]>=sum(P_unit_mins[j]*L_unit[j][i,t] for j in range(N_unit_segs))]
        constraints+=[P_unit[i,t]<=sum(P_unit_maxs[j]*L_unit[j][i,t] for j in range(N_unit_segs))]

        #有且只有一段被激活,均不激活的时候为P_unit为0
        constraints+=[sum(L_unit[j][i,t] for j in range(N_unit_segs))<=U_unit[i,t]]
        ...
        #最终运行成本
        constraints+=[Cost_unit_opr[i,t]==sum(Cost_unit_opr_per_seg[j][i,t] for j in range(N_unit_segs))]
        '''
...
for i in range(N_ESs):
    ...
    for t in range(T):
        ...
        #放电功率分段上下限约束
        constraints+=[P_ES_dch[i,t]>=sum(P_ES_mins[j]*L_ES[j][i,t] for j in range(N_ES_segs))]
        constraints+=[P_ES_dch[i,t]<=sum(P_ES_maxs[j]*L_ES[j][i,t] for j in range(N_ES_segs))]


        #有且只有一段被激活
        constraints+=[sum(L_ES[j][i,t] for j in range(N_ES_segs))<=U_dch[i,t]]
        ...
        #最终放电成本
        constraints+=[Cost_ES_dch[i,t]==sum(Cost_ES_dch_per_seg[j][i,t] for j in range(N_ES_segs))]
